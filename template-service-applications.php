<?php
/* Template Name: Page service — Applications */
/**
 * Gabarit dédié à la page « Applications web sur mesure ».
 *
 * Contrairement aux 2 autres pages service qui utilisent le gabarit générique
 * (template-service.php), cette page a 2 sections uniques en plus des
 * sections communes : le split « Pourquoi des applications sur mesure? » et
 * la grille des 6 tuiles « Types d'applications que nous développons ».
 *
 * Sections uniques : champs ACF préfixés `svcapp_` (groupe ACF
 * « Page service — Applications (sections uniques) », inc/acf-fields-services-extra.php).
 ...
 * pas redéfinis.
 *
 * Valeurs par défaut PHP = contenu réel de service-applications.html (maquette
 * approuvée), pour que la page affiche déjà le bon contenu dès sa création.
 *
 * @package KPIBI
 */
...
while ( have_posts() ) :
	...
	/* ----- Valeurs par défaut des sections UNIQUES (contenu de service-applications.html) ----- */

	$kpibi_svcapp_pourquoi_img = kpibi_f( 'svcapp_pourquoi_image', '' );

	$kpibi_svcapp_tuiles_defaults = array(
		array(
			'titre' => 'Ventes et CRM',
			'texte' => 'Outils de gestion des ventes et CRM adaptés au terrain, pensés pour la réalité de vos équipes.',
			'icone' => 'equipe',
		),
		array(
		...
		array(
			'titre' => 'Gestion des commandes et de la production',
			'texte' => 'Plateformes de gestion des commandes et de la production, du devis jusqu\'à la livraison.',
			'icone' => 'cube',
		),
		array(
			'titre' => 'Gestion des demandes et approbations',
			'texte' => 'Applications de gestion des demandes internes et des flux d\'approbation, du dépôt jusqu\'à la décision.',
			'icone' => 'check',
		),
		array(
			'titre' => 'Planification et gestion de projets',
			'texte' => 'Systèmes de planification et de gestion de projets qui optimisent vos ressources et vos échéanciers.',
			'icone' => 'calendrier',
		),
		array(
			'titre' => 'Intégrations ERP et systèmes d\'affaires',
			'texte' => 'Applications intégrées directement à vos ERP, CRM et systèmes d\'affaires existants, sans rupture.',
			'icone' => 'sync',
		),
	);
	...
	<section class="page-hero">
		...
		<div class="container"><div class="page-hero-inner">
			...
			<div class="page-hero-actions">
				<a href="<?php echo esc_url( kpibi_f( 'service_hero_cta1_url', '#cta' ) ); ?>" class="btn btn-primary"><?php echo esc_html( kpibi_f( 'service_hero_cta1_texte', 'Planifier une consultation gratuite' ) ); ?></a>
				<a href="<?php echo esc_url( kpibi_link( kpibi_f( 'service_hero_cta2_url', '' ), 'cas-clients' ) ); ?>" class="btn btn-outline"><?php echo esc_html( kpibi_f( 'service_hero_cta2_texte', 'Voir nos réalisations' ) ); ?></a>
			</div>
		</div></div>
	</section>

	<!-- SECTION UNIQUE — POURQUOI DES APPLICATIONS SUR MESURE? -->
	<section class="section-split bg-light">
		<div class="container"><div class="split-inner">
			...
			<div class="split-content reveal reveal-delay-1">
				<p class="section-label"><?php echo esc_html( kpibi_f( 'svcapp_pourquoi_label', 'Pourquoi sur mesure?' ) ); ?></p>
				<h2><?php
					echo esc_html( kpibi_f( 'svcapp_pourquoi_titre', 'Pourquoi des applications' ) ) . ' ';
					echo '<strong>' . esc_html( kpibi_f( 'svcapp_pourquoi_titre_fort', 'sur mesure?' ) ) . '</strong>';
				?></h2>
				<p><?php echo esc_html( kpibi_f( 'svcapp_pourquoi_texte1', "La plupart des entreprises finissent par tordre leurs processus pour les faire entrer dans un logiciel générique conçu pour des milliers d'autres compagnies." ) ); ?></p>
				<p><?php echo esc_html( kpibi_f( 'svcapp_pourquoi_texte2', "Résultat : Excel parallèles, doubles saisies, contournements, perte d'information, frustration et manque de visibilité." ) ); ?></p>
				<p><?php
					echo esc_html( kpibi_f( 'svcapp_pourquoi_texte3_debut', "Chez KPIBI, on fait l'inverse. Nous créons des applications taillées sur mesure pour vos processus, vos équipes et vos objectifs. L'idée n'est pas d'ajouter de la technologie pour le plaisir, mais de concevoir un système qui" ) ) . ' ';
					echo '<strong>' . esc_html( kpibi_f( 'svcapp_pourquoi_texte3_fort', 'rend la performance naturelle.' ) ) . '</strong>';
				?></p>
			</div>
		</div></div>
	</section>

	<!-- SECTION UNIQUE — GRILLE DES 6 TUILES : TYPES D'APPLICATIONS -->
	<section class="section-steps" style="background:var(--slate-50);">
		<div class="container">
			<div class="section-header reveal">
				<p class="section-label"><?php echo esc_html( kpibi_f( 'svcapp_tuiles_label', 'Ce que nous développons' ) ); ?></p>
				<h2><?php
					echo esc_html( kpibi_f( 'svcapp_tuiles_titre', "Types d'applications que" ) ) . ' ';
					echo '<strong>' . esc_html( kpibi_f( 'svcapp_tuiles_titre_fort', 'nous développons' ) ) . '</strong>';
				?></h2>
				<p><?php echo esc_html( kpibi_f( 'svcapp_tuiles_intro', "Nous développons principalement des applications qui touchent le cœur des opérations. Si vous gérez encore ce processus dans Excel, c'est probablement un bon candidat." ) ); ?></p>
			</div>
			<?php
			...
			$kpibi_tuiles_class = ( count( $kpibi_svcapp_tuiles_items ) > 4 ) ? 'steps-grid tiles cols3' : 'steps-grid tiles';
			?>
			<div class="<?php echo esc_attr( $kpibi_tuiles_class ); ?>">
				<?php foreach ( $kpibi_svcapp_tuiles_items as $kpibi_i => $kpibi_item ) : ?>
					<?php $kpibi_icone = ! empty( $kpibi_item['icone'] ) ? $kpibi_item['icone'] : $kpibi_svcapp_tuiles_defaults[ $kpibi_i % 6 ]['icone']; ?>
					<div class="step-card reveal<?php echo $kpibi_i > 0 ? ' reveal-delay-' . esc_attr( min( $kpibi_i, 4 ) ) : ''; ?>">
						<div class="benefit-icon" style="background:rgba(212,175,106,0.10);margin-bottom:16px;">
							<svg viewBox="0 0 24 24" style="width:24px;height:24px;stroke:var(--gold-600);fill:none;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round;"><?php echo kpibi_icon( $kpibi_icone ); /* phpcs:ignore -- bibliothèque d'icônes interne, pas d'entrée utilisateur libre */ ?></svg>
						</div>
